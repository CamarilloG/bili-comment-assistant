...
async function startTask() {
  ...
  try {
    ...
    if (mode.value === 'comment') {
      ...
      console.log('[DEBUG] 调用 startDm')
      response = await taskApi.startDm(slot)
    }
...
const modes = [
  ...
  { value: 'dm', label: '私信模式', desc: '自动私信目标用户', color: 'green' },
]
</script>
    ...
    <section class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-4 gap-3">
        <label
          ...
          :class="mode === m.value
            ? (m.color === 'blue' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : m.color === 'purple' ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20' : m.color === 'green' ? 'border-green-500 bg-green-50 dark:bg-green-900/20' : 'border-orange-500 bg-orange-50 dark:bg-orange-900/20')
      ...
      <div class="flex gap-3 mt-5">
        <button
          ...
          :class="mode === 'warmup' ? 'bg-orange-500 hover:bg-orange-600' : mode === 'ai_comment' ? 'bg-purple-600 hover:bg-purple-700' : mode === 'dm' ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'"
        >
